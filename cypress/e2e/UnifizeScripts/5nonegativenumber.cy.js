describe('testscripts on unifize app', () => {
    ...
    it.only('Creating Process and creating a chatroom and Testing the number with min 20 and max 100 ', () => {
        ...
        cy.get('header > .chakra-input').type('Testing the number with no negative number within 100')
        cy.get('#description').type('Testing the number with negative number')
        ...
        cy.get('.css-1e4rb2a').contains('Testing the number with no negative number within 100').click()
        cy.get(8000)
        cy.get('.css-aggl2l > :nth-child(3)').click()
        cy.get('#title').type('validation for the number field with negative number')
    ...
    //     //ADDING NUMBER IN NUMBERFIELD
          cy.get('.css-mze1nh > :nth-child(1) > .chakra-input').type('Enter Number with no negative number')
    ...
    cy.wait(3000)
        ...
        cy.get('.css-1is2g17').click()
        cy.get('.css-yncqvq').type('19.9999')
        cy.get('.css-1q5hvdi').click()
        cy.get('.css-10zelb5').dblclick()

        cy.get('.css-1is2g17').click()
       cy.get('.css-yncqvq').type('70.897699')
       cy.get('.css-1q5hvdi').click()
