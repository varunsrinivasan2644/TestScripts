describe('testscripts on unifize app', () => {
    ...
    it.only('Testing the number with floating point upto 3 with negative values', () => {
        ...
        cy.get('header > .chakra-input').type('Testing the number with floating values with negative values')
        cy.get('#description').type('Testing the number with floating point upto 3 with negative values')
        ...
        cy.get('.css-1e4rb2a').contains('Testing the number with floating values with negative values').click()
        cy.get(8000)
        cy.get('.css-aggl2l > :nth-child(3)').click()
        cy.get('#title').type('Testing the number field with floating points with negative values')
    ...
    //     //ADDING NUMBER IN NUMBERFIELD
          cy.get('.css-mze1nh > :nth-child(1) > .chakra-input').type('Enter Number with negative values with floating point 3(-5 to 100)')
       ...
       cy.get('#max').type('100')
       cy.get('#min').type('-5')
       cy.get('.css-owjkmg > :nth-child(2) > .chakra-stack').click()
       cy.get('#formatDecimalPlaces').type('3')
       cy.get(':nth-child(4) > label > div').click()
    ...
    cy.wait(3000)
        ...
        cy.get('.css-1is2g17').click()
       cy.get('.css-yncqvq').type('-2.897656')
       cy.wait(2000)
       cy.get('.css-1q5hvdi').click()
      })
    })
