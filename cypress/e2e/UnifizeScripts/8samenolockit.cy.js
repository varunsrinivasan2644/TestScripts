describe('testscripts on unifize app', () => {
    ...
    it.only('Hiding the checklist if number is above 100 ', () => {
        ...
        cy.get('header > .chakra-input').type('Number field is blocked when the number is equal to 10')
        cy.get('#description').type('If number is equal to 10 checklist should be locked')
       ...
       // cy.get(':nth-child(3) > .css-ncn170 > [style="display: flex; align-items: center; gap: 0.5rem;"] > a > .chakra-text').click()
        ...
        cy.get('#title').type('Number field is blocked when the number is equal to 10')
    ...
    //     //ADDING NUMBER IN NUMBERFIELD
          cy.get('.css-mze1nh > :nth-child(1) > .chakra-input').type('Enter a number equal to  10')
    ...
    //    //qualifier
        ...
        cy.get('.css-1fzly1n > .sc-gsFSXq > .sc-imWYAI').type('10')
